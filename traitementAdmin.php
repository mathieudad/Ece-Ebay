...
function affichageAdmin($type, $id){

  if($type == "Client" || $type == "Vendeur"){
    echo "Vous etes $type, cette page est reservée aux Admins";
    return;
  }
  ...
  if ($db_found) {
    $sqlVendeur = "SELECT * FROM vendeur";
    $resultVendeur = mysqli_query($db_handle, $sqlVendeur);
    if (mysqli_num_rows($resultVendeur) == 0) {
      //le livre recherché n'existe pas
      echo "Aucun Vendeur";
    }else {
      echo "<h3>Ajouter un nouveau vendeur</h3><hr>";
      afficheFormulaireVendeur();
      echo "<h3>Liste des Vendeurs</h3><hr>";
